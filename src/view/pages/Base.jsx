...
export default function Base({ title, filters = [], model, form, data }) {
    ...
    return user.appsync ? <>
        ...
        <Divider />
        <BaseTable data={tableData} title={filter?.name || title} columns={form?.read?.fields || []} schema={form?.schema || {}} form={form} actions={form?.read?.actions || []} model={model} actionCallback={async () => setTableData(await getTableData())} />
